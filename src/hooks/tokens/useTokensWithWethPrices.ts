import { ChainId, WETH, Token, Blockchain } from '@deathswap/sdk'
...
export default function useTokensWithWethPrices(): Record<string, any> {
  ...
  const bscBUSD: Token | undefined = blockchain === Blockchain.FANTOM ? getToken(chainId, 'bscBUSD') : undefined
  let bscBUSDWETHPrice = useTokenWethPrice(bscBUSD)
  bscBUSDWETHPrice = bscBUSDWETHPrice ? bscBUSDWETHPrice : BUSDWETHPrice

  const bscUSDC: Token | undefined = blockchain === Blockchain.FANTOM ? getToken(chainId, 'bscUSDC') : undefined
  let bscUSDCWETHPrice = useTokenWethPrice(bscUSDC)
  bscUSDCWETHPrice = bscUSDCWETHPrice ? bscUSDCWETHPrice : USDCWETHPrice

  const bscDAI: Token | undefined = blockchain === Blockchain.FANTOM ? getToken(chainId, 'bscDAI') : undefined
  let bscDAIWETHPrice = useTokenWethPrice(bscDAI)
  bscDAIWETHPrice = bscDAIWETHPrice ? bscDAIWETHPrice : DAIWETHPrice

  const bridgedETH: Token | undefined = Blockchain.FANTOM ? getToken(chainId, '1ETH') : undefined
  const bridgedETHWETHPrice = useTokenWethPrice(bridgedETH)

  const bridgedBscETH: Token | undefined = Blockchain.FANTOM ? getToken(chainId, 'bscETH') : undefined
  ...
  const bridgedBTC: Token | undefined = Blockchain.FANTOM ? getToken(chainId, '1WBTC') : undefined
  const bridgedBTCWETHPrice = useTokenWethPrice(bridgedBTC)

  const bridgedBscBTC: Token | undefined = Blockchain.FANTOM ? getToken(chainId, 'bscBTCB') : undefined
